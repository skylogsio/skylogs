...
import type { AlertRuleStatus, IAlertRule, IElasticAlertHistory } from "@/@types/alertRule";
...
export default function ElasticAlertHistory({ alertId }: { alertId: IAlertRule["id"] }) {
  const { palette } = useTheme();

  const { data, fetchNextPage, hasNextPage, isFetching, isFetchingNextPage } = useInfiniteQuery({
    queryKey: ["elastic-alert-rule-history", alertId],
    queryFn: ({ pageParam }) => getAlertRuleHistory<IElasticAlertHistory>(alertId, pageParam),
    initialPageParam: 1,
  ...
  return (
    ...
    >
      <DataTable<IElasticAlertHistory>
        data={allData}
        columns={[
