...
final class ElasticStatusEventSource implements AlertStatusEventSource
{
    ...
    public function fetchBaseline(Collection $alertRules, Carbon $before): Collection
    {
        ...
        foreach ($alertRules as $alertRuleId => $alertRule) {
            ...
            if ($check !== null && (int) $check->state === ElasticCheck::FIRE) {
    ...
    /**
     * @param  Collection<string, AlertRule>  $alertRules
     * @return Collection<string, ElasticCheck>
     */
    private function checksForAlertRules(Collection $alertRules): Collection
    {
        return ElasticCheck::query()
            ->whereIn('alertRuleId', $alertRules->pluck('_id')->all())
            ->get()
            ->keyBy(fn (ElasticCheck $check): string => (string) $check->alertRuleId);
    }

    private function shouldReconcileAsFire(AlertStatusEvent $baselineEvent, ?ElasticCheck $check): bool
    {
        return $check !== null
            && (int) $check->state === ElasticCheck::FIRE
    ...
    private function resolveOpenFireTimestamp(
        ...
        $fireDocument = ElasticHistory::query()
            ->where('alertRuleId', $alertRule->_id)
            ->where('state', ElasticHistory::FIRE)
    ...
    private function syntheticFireEvent(AlertRule $alertRule, ElasticCheck $check, Carbon $occurredAt): AlertStatusEvent
    {
        $fireDocument = ElasticHistory::query()
            ->where('alertRuleId', $alertRule->_id)
            ->where('state', ElasticHistory::FIRE)
        ...
        return new AlertStatusEvent(
            ...
            summary: sprintf(
                ...
                $check->dataviewTitle ?? $alertRule->dataviewTitle ?? $check->dataviewName ?? $alertRule->dataviewName ?? '',
            ),
        );
    }
